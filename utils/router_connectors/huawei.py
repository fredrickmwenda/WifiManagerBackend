...
class HuaweiConnector(RouterConnector):
    """
    Huawei VRP routers / AR series via SSH (Netmiko).
    """
    ...
    def get_system_info(self):
        try:
            output = self.connection.send_command("display version", use_textfsm=True)
            if isinstance(output, list) and len(output) > 0:
                d = output[0]
                return {
                    "name": d.get("hostname", self.router.name),
                    "firmware": d.get("vrp_version", ""),
                    "uptime": d.get("uptime", ""),
    ...
    def get_dhcp_leases(self):
        try:
            output = self.connection.send_command(
                "display dhcp server lease", use_textfsm=True
            )
                ...
                return [
                    {
                        ...
                        "host_name": o.get("hostname") or "Unknown",
                        "status": "active",
                    }
                    for o in output
                ]
    ...
    def reboot(self):
        try:
            self.connection.send_command_timing("reboot", strip_prompt=False)
            self.connection.send_command_timing("Y", strip_prompt=False)
            return True
        except Exception:
            return False
    ...
    def block_mac(self, mac_address: str):
        try:
            self.connection.send_command_timing(
                f"mac-address blackhole {mac_address} vlan 1"
            )
            return True
        except Exception:
            return False
    ...
    def unblock_mac(self, mac_address: str):
        try:
            self.connection.send_command_timing(
                f"undo mac-address blackhole {mac_address} vlan 1"
            )
            return True
        except Exception:
            return False